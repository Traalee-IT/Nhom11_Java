	    ...
	    if(this.rdbtnNam.isSelected()) {
		 ...
		 if(mon1<0||mon1>10||mon2<0||mon3<0||mon3>10||mon2>10) {
				...
				if(luachon==JOptionPane.YES_OPTION) {
		 		...
		 		// cập nhật vào table và cập nhật vào cơ sỏ dữ liệu cho bảng sinh viên 
	 ...
	 */
        ...
        if(DanhSachDoituong()!=null) {
			...
			for(Thisinh SV: DanhSachDoituong()) {
	        	...
	        	fw.write(SV+"\n"); ;// đọc dữ liẹu từ mangr đối tượng và sau dau đó thưucj hiện dộ từng dingf theo từng lần lặp của 
...
public void addAllDataBaseOntoTable(){
	...
	// sở dữ liêu
		   ...
		   DefaultTableModel model=(DefaultTableModel)table_1.getModel();
		model.setRowCount(0);// hàm ânyf set cho table có 0 hàng 
     }
}